...
flowchart TD
    ...
    C -->|调度| D["Model Worker"]:::worker
    D -->|加载| E["大模型"]:::model
    B -->|查询| F["数据库"]:::db
    B -->|查询| G["向量数据库"]:::vector_db
    B -->|查询| H["知识库"]:::kb
    ...
    classDef db fill:#FECA57,stroke:#2D3436,stroke-width:2px,color:#2D3436,rx:8,ry:8;
    classDef vector_db fill:#96CEB4,stroke:#2D3436,stroke-width:2px,color:#2D3436,rx:8,ry:8;
    classDef kb fill:#E9ECEF,stroke:#2D3436,stroke-width:3px,color:#2D3436,rx:8,ry:8;
    
    Note over A,H: 数据流: 客户端请求 → API Server处理 → 调用相应服务 → 返回结果
```
...
flowchart TD
    ...
    B -->|请求| C["Controller"]:::controller
    C -->|调度| D["Model Worker集群"]:::worker_cluster
    D -->|加载| E["大模型"]:::model
    B -->|查询| F["数据库集群"]:::db_cluster
    B -->|查询| G["向量数据库集群"]:::vector_db_cluster
    B -->|查询| H["分布式文件系统"]:::dfs
    
    classDef client fill:#FF6B6B,stroke:#2D3436,stroke-width:3px,color:white,rx:8,ry:8;
    classDef api_cluster fill:#45B7D1,stroke:#2D3436,stroke-width:2px,color:white,rx:8,ry:8;
    classDef controller fill:#54A0FF,stroke:#2D3436,stroke-width:2px,color:white,rx:8,ry:8;
    classDef worker_cluster fill:#4ECDC4,stroke:#2D3436,stroke-width:2px,color:#2D3436,rx:8,ry:8;
    classDef model fill:#FF9FF3,stroke:#2D3436,stroke-width:2px,color:#2D3436,rx:8,ry:8;
    classDef db_cluster fill:#FECA57,stroke:#2D3436,stroke-width:2px,color:#2D3436,rx:8,ry:8;
    classDef vector_db_cluster fill:#96CEB4,stroke:#2D3436,stroke-width:2px,color:#2D3436,rx:8,ry:8;
    classDef dfs fill:#E9ECEF,stroke:#2D3436,stroke-width:3px,color:#2D3436,rx:8,ry:8;
    
    Note over A,H: 数据流: 客户端请求 → API Server集群 → 多组件协同 → 返回结果
```
