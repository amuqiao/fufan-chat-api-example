...
def check_model_exists(local_dir):
    ...
    if not has_model_weight:
        has_sharded_model = any("pytorch_model-" in file and "-of-" in file for file in all_files)
    ...
    tokenizer_files = ["tokenizer.json", "tokenizer_config.json", "vocab.txt", "sentencepiece.bpe.model", "tokenizer.model"]
...
def test_model_import(model_config, verify_level="full"):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"⚠️ accelerate库不可用，不使用device_map='auto' (可通过 'pip install accelerate' 安装以优化大模型加载)")
        ...
        if model_config.get("is_llm"):
            ...
            if verify_level == "full":
                ...
                try:
                    ...
                    logger.error(f"⚠️ LLM生成功能测试失败: {model_config['name']}, 错误: {func_error}")
                    logger.error(f"  💡 提示: 功能测试失败不影响模型使用，可能是环境或参数问题")
            ...
            if verify_level == "full":
                ...
                try:
                    ...
                    func_test_success = False
                    logger.error(f"⚠️ 嵌入生成功能测试失败: {model_config['name']}, 错误: {func_error}")
                    logger.error(f"  💡 提示: 功能测试失败不影响模型使用，可能是环境或参数问题")
            ...
            if verify_level == "full":
                ...
                try:
                    ...
                    func_test_success = False
                    logger.error(f"⚠️ 重排序功能测试失败: {model_config['name']}, 错误: {func_error}")
                    logger.error(f"  💡 提示: 功能测试失败不影响模型使用，可能是环境或参数问题")
...
def download_model(model_config):
    ...
    try:
        ...
        logger.info(f"✓ 模型下载完成: {model_config['name']}, 保存路径: {downloaded_dir}")
        return True
    except Exception as e:
        error_msg = str(e)
        ...
        if "not exists on either" in error_msg or "<Response [404]" in error_msg:
            logger.error(f"  💡 失败原因: 模型 {model_config['model_id']} 不在ModelScope平台上")
            logger.error(f"  💡 解决方案:")
        ...
        if os.path.exists(model_dir):
            try:
                ...
                logger.error(f"  ✗ 清理模型文件夹失败: {model_dir}, 错误: {cleanup_error}")
...
def main(verify_level="full"):
    ...
    for i, model_config in enumerate(MODELS_CONFIG, 1):
        ...
        if check_model_exists(model_config["local_dir"]):
            ...
            # 2. 测试模型是否能够导入
            try:
                import_success, func_test_success = test_model_import(model_config, verify_level)
                ...
                if import_success:
                    ...
                    if not func_test_success:
                        logger.warning(f"⚠️  模型导入成功，但功能测试失败 (不影响模型使用)")
                    continue
                else:
                ...
                if any(keyword in error_msg for keyword in ["not found in your environment", "requires the", "missing", "no module named"]):
